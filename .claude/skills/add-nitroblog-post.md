---
description: Add a blog post to a NitroBlog-AI-powered site. Triggers when the user says "add a post", "publish this article", "Arc handed me a draft", "write a post about X for <site>", "draft a blog post on Y", or hands you Markdown/MDX with instructions to publish. Handles every SEO technicality (frontmatter, headings, internal links, FAQ schema, alt text, slug, meta lengths) so the agent can take a draft and ship a publication-ready post without follow-up.
---

# Skill — Add a Blog Post to a NitroBlog AI

You are publishing a blog post to a site running `@nitroblog/core`. The end state: a new MDX file in the per-site repo's `_blog/src/content/posts/`, committed and pushed, with **every SEO technicality correct** — so the post is genuinely ready to perform, not just published.

This skill assumes the engine is already integrated into the site (use [integrate-nitroblog.md](integrate-nitroblog.md) for that). If `_blog/src/content/posts/` doesn't exist for the target site, stop and run the integration skill first.
...
Open `_blog/src/content/posts/<slug>.mdx`. The frontmatter Zod schema is in `@nitroblog/core/schemas/post`. Fill **every applicable field**:
...
**Safety net**: `nitroblog-doctor` walks the built `dist/` post pages and **fails fatal if any built post page has more than one `<h1>`**. If the plugin somehow doesn't run (e.g. someone removed `@nitroblog/core/integration` from `astro.config.ts`), this catches the regression before completion.
...
The engine exports a programmatic helper from `@nitroblog/core`:

```ts
import { suggestInternalLinks } from "@nitroblog/core"
...
npx nitroblog-doctor heroes
# 2. Read art direction if it exists:
cat .nitroblog/art-direction.json 2>/dev/null
```

### 7.5a — First post on this site (no art direction yet)

If `.nitroblog/art-direction.json` doesn't exist, you must establish art direction BEFORE generating. The agent prompts the user — exact text:
...
Write the file at **`<site-repo>/.nitroblog/art-direction.json`** following the engine's `artDirectionSchema` (export from `@nitroblog/core`). Required fields: `style`, `aspectRatio` (default "16:9"), `createdAt` (ISO timestamp), `source`. Optional: `palette`, `subjectHint`, `avoid`, `notes`.
...
5. **Update frontmatter** with:
   ...
   ```
6. **Verify**: `npx nitroblog-doctor heroes` should no longer list this slug.
...
The host's `nitroblog-updater.php` cron will pull the new release within 24 hours (random time between 23:00–02:00 — see install). If you want it live faster:

```bash
# Force the host to update now:
curl "https://<domain>/nitroblog-updater.php?token=<TOKEN>"
```

(Token is in `~/.nitroblog-updater.config.php` on the host.)
...
- [ ] `.nitroblog/art-direction.json` exists for the site (created in Phase 7.5a on first post if not already present)
