...
// packages/create-nitroblog/bin/create.mjs -> ../../examples/demo-site
...
async function main() {
  ...
  if (!existsSync(TEMPLATE_DIR)) {
    ...
    console.error("  This CLI must be run from inside the nitroblog-ai repo,")
    console.error("  or installed where the template is co-located.")
    process.exit(1)
  }

  console.log(`\nNitroBlog AI — scaffolder\n`)
  ...
  const newConfig = `import type { SiteConfig } from "@nitroblog/core"
...
`
  ...
  pkg.name = `@nitroblog/${slugify(answers.siteName)}`
  pkg.private = true
  delete pkg.dependencies["@nitroblog/core"]
  delete pkg.dependencies["@nitroblog/theme-default"]
  pkg.dependencies["@nitroblog/core"] = "git+ssh://git@github.com:YOUR_ORG/nitroblog-ai.git#workspace=@nitroblog/core"
  pkg.dependencies["@nitroblog/theme-default"] = "git+ssh://git@github.com:YOUR_ORG/nitroblog-ai.git#workspace=@nitroblog/theme-default"
