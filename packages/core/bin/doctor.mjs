#!/usr/bin/env node
/**
 * nitroblog-doctor — preflight + post-install integration checker + closeout gate.
 *
 ...
 *   npx nitroblog-doctor
 *
 *   # Reduced modes (use during development, not for declaring done)
 *   npx nitroblog-doctor --preflight       # install-level only
 *   npx nitroblog-doctor --lite            # technical only, no procedural gates
 *   npx nitroblog-doctor --skip-build      # skip the build check (faster)
 *   npx nitroblog-doctor --json            # machine-readable output
 *
 *   # Attestations — write entries to .nitroblog-integration-state.json
 *   # Without these, default doctor mode FAILS — you cannot declare done.
 *   npx nitroblog-doctor attest-phase9 '<json-string-of-answers>'
 *   npx nitroblog-doctor attest-feedback provided <FEEDBACK-FOR-CLAUDE-name.md>
 *   npx nitroblog-doctor attest-feedback none-needed "<short reason>"
 *   npx nitroblog-doctor attest-nav-link approved
 *   npx nitroblog-doctor attest-nav-link declined
 *
...
// nitroblog-doctor and the OpenClaw-side runtime skill changes
...
//
// Runtime skill at ~/.openclaw/skills/nitroblog-final-gate/SKILL.md should
...
// ---------- state file (.nitroblog-integration-state.json) ----------
//
...
const STATE_FILE = join(CWD, ".nitroblog-integration-state.json")
...
function attestPhase9(args) {
  if (args.length === 0) {
    console.error(`Usage: nitroblog-doctor attest-phase9 '<json-answers>'
...
function attestFeedback(args) {
  ...
  if (status === "provided") {
    const file = args[1]
    if (!file) {
      console.error(`Usage: nitroblog-doctor attest-feedback provided <FEEDBACK-FOR-CLAUDE-name.md>`)
      process.exit(1)
    }
    ...
    if (!reason || reason.length < 10) {
      console.error(`Usage: nitroblog-doctor attest-feedback none-needed "<at-least-10-char honest reason>"
...
`)
    ...
    console.error(`Usage: nitroblog-doctor attest-feedback <provided|none-needed> ...`)
    process.exit(1)
  }
}
...
function attestNavLink(args) {
  ...
  if (decision !== "approved" && decision !== "declined") {
    console.error(`Usage: nitroblog-doctor attest-nav-link <approved|declined>
...
Run 'npx nitroblog-doctor' to see what's blocking. Fix the failures,
...
`)
  ...
  // Engine version from core
  const enginePkg = resolvePackageJson("@nitroblog/core", CWD)
...
- nitroblog-doctor: ✓ all checks passed (default mode = full closeout gate)
...
//
//   npx nitroblog-doctor heroes           # human-readable
//   npx nitroblog-doctor heroes --json    # machine-readable
function heroesReport(opts = {}) {
  ...
  const adCandidates = ["..", "."]
    .map((p) => resolve(CWD, p, ".nitroblog/art-direction.json"))
  ...
  if (artDirection) {
    ...
    console.log("No .nitroblog/art-direction.json found yet — on first hero generation, prompt")
    console.log("the user for art direction or screenshot the homepage to derive one, then")
    console.log("write the file at <site-repo>/.nitroblog/art-direction.json.")
  }
}
...
for (const pkg of ["@nitroblog/core", "@nitroblog/theme-default"]) {
...
if (!tokensPath) {
  fail(
    ...
    "Phase 2 of the integration skill produces this file. Without it, the blog uses engine defaults (system-ui font, neutral colours) " +
      "and won't visually match the host. Run Phase 2 before declaring done. See packages/theme-default/TOKEN-CONTRACT.md.",
  )
} else {
...
for (const cssPath of globalCssCandidates) {
  ...
  if (/@import\s+["'].*@nitroblog\/theme-default\/tokens\.css/.test(c)) {
    fail(
      "global.css imports the engine's tokens.css",
      `${cssPath.replace(CWD, ".")} contains '@import "@nitroblog/theme-default/tokens.css"'. ` +
  ...
  for (const postsDir of heroPostsDirs) {
    ...
    if (offenders.length === 0) {
      ...
      const adHint = adPath
        ...
        : `No .nitroblog/art-direction.json found yet — running the hero generation flow will create one on first use.`
      fail(
...
// 5e. Popularity data staleness (v0.3.12+). If .nitroblog/popularity.json
...
if (!PREFLIGHT) {
  const popCandidates = ["..", "."]
    .map((p) => resolve(CWD, p, ".nitroblog/popularity.json"))
    ...
    if (offenders.length === 0) {
      ...
      fail(
        ...
        `Multi-H1 pages confuse crawlers + screen readers + the ToC. ` +
          ...
          `check that @nitroblog/core/integration is loaded in astro.config ` +
      ...
      if (missing.length > 0) {
        fail(
          ...
          `Missing: ${missing.join(", ")}. ` +
            ...
            `usually because file: deps are broken (check #1 above) so node_modules/@nitroblog/core can't be scanned.`,
        )
      } else {
  ...
  if (!state || !state.phase9?.asked) {
    fail(
      ...
      `Default doctor (full closeout gate) requires evidence that you asked the user the Phase 9 questions. ` +
        `Run: npx nitroblog-doctor attest-phase9 '<json-of-answers>'\n` +
  ...
  if (!state || !state.navLink?.decision) {
    fail(
      ...
      `Phase 7b requires asking the user whether to add a 'Blog' link to the main nav (vs footer-only). Then attest the answer:\n` +
        `  npx nitroblog-doctor attest-nav-link approved   # if user said yes AND you added the link to host nav\n` +
        `  npx nitroblog-doctor attest-nav-link declined   # if user said no\n` +
