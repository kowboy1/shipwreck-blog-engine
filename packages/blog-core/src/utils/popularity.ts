 ...
 *   2. .shipwreck/popularity.json (analytics-derived ranking, if present)
...
/**
 * Walk up from cwd looking for `.shipwreck/popularity.json`. Mirrors the
 * art-direction lookup convention used by doctor: checks `./.shipwreck/...`
 * and `../.shipwreck/...` to handle both "run from site root" and "run from
 * _blog/" layouts.
 */
export function loadPopularityFile(cwd: string = process.cwd()): PopularityLoadResult {
  const candidates = [".", ".."].map((p) => resolve(cwd, p, ".shipwreck/popularity.json"))
